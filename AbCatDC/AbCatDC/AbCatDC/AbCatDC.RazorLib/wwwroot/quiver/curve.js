...
/// A flat symmetric quadratic B̩zier curve.
export class Bezier extends Curve {
    ...
    /// Returns the B̩zier curve from t = 0 to t = `t` as a series of points corresponding
    ...
    /// Returns the arc length of the B̩zier curve from t = 0 to t = `t`.
    /// These B̩zier curves are symmetric, so t = `t` to t = 1 can be calculated by inverting the
    /// arc length from t = 0.
    arc_length(t) {
    ...
    /// B̩zier curve. (It returns a function, rather than the t for a length, to allow the segments
    ...
    /// Intersect the B̩zier curve with the given rounded rectangle. Note that the general
    /// (analytic) problem of intersecting a B̩zier curve with a circle (for the rounded corners) is
    /// very difficult, so we approximate circles with regular polygons. If the rounded rectangle
    /// entirely contains the B̩zier curve, and `permit_containment` is true, a single intersection
    ...
    intersections_with_rounded_rectangle(rect, permit_containment) {
        ...
        // B̩zier curve is zero (i.e. the curve is a straight line). We special-case this type of
        // curve, and do not normalise its height.
        const h = this.h || 1;

        // Normalise all the points with respect to the B̩zier curve. From this point on, we do
        ...
        if (this.h === 0) {
            ...
            // The usual case: when we have a nontrivial B̩zier curve.
            for (let i = 0; i < points.length; ++i) {
                const endpoints = [points[i], points[(i + 1) % points.length]];
                if (Math.abs(endpoints[0].x - endpoints[1].x) <= EPSILON) {
                    // `x = a`.
        ...
        return Array.from(intersections).map((p) => {
            // The derivative of the normalised B̩zier curve is `2 - 4x`.
            return new CurvePoint(p.scale(this.w, h), p.x, Math.atan2((2 - 4 * p.x) * h, this.w));
        });
    }

    /// Render the B̩zier curve to an SVG path.
    render(path) {
        return path.curve_by(new Point(this.w / 2, this.h), new Point(this.w, 0));
    }
}

/// A point on a quadratic B̩zier curve or arc, which also records the parameter `t` and the tangent
...
/// A quadratic B̩zier curve whose endpoints are `(0, 0)` and `(1, 0)` and whose control point
...
/// A very simple class for computing the value of a cubic B̩zier at a point, using for replicating
...
export class Arc extends Curve {
    ...
    intersections_with_rounded_rectangle(rect, permit_containment) {
        ...
        // B̩zier code, which already special cases straight lines. Since the circles involved can
