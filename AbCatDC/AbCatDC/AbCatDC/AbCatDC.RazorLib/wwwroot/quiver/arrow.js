import { Arc, Bezier, Curve, CurvePoint, EPSILON, RoundedRectangle } from "./curve.js";
import { DOM } from "./dom.js";
import { Dimensions, Enum, Path, Point, rad_to_deg, clamp } from "./ds.js";
...
export const CONSTANTS = {
    ...
    /// The length of the line segments in an adjunction symbol (âŠ£).
    ...
    ARROW_SHAPE: new Enum(
        "ARROW_SHAPE",
        // BÃ©zier curve (default for edges).
        "BEZIER",
        // Arc (used for loops).
        "ARC",
    ),
    ...
    ARROW_BODY_STYLE: new Enum(
        ...
        // No edge ( ).
        "NONE",
        // A line (â€”). This is the default.
        "LINE",
        // A squiggly line, made up of alternating triangles (âŸ¿).
        "SQUIGGLY",
        // The adjunction symbol: âŠ£.
        "ADJUNCTION",
    ...
    ARROW_DASH_STYLE: new Enum(
        "ARROW_DASH_STYLE",
        // A solid line, with no breaks (â€”). This is the default.
        "SOLID",
        // A dashed line (â¤).
        "DASHED",
        // A dotted line (â¤‘).
    ...
    ARROW_HEAD_STYLE: {
        /// No arrow head (-).
        NONE: [],
        /// The usual arrow head style (â†’).
        NORMAL: ["epi"],
        /// A double arrow head (â† ).
        EPI: ["epi", "epi"],
        /// A reversed arrow head (â¤š).
        MONO: ["mono"],
        /// A perpendicular line (â†¦).
        MAPS_TO: ["maps to"],
        /// A harpoon: just the upper part of the arrow head (â‡€).
        HARPOON_TOP: ["harpoon-top"],
        /// A harpoon: just the lower part of the arrow head (â‡).
...
export class ArrowStyle {
    constructor() {
        ...
        // BÃ©zier curves and arcs. However, `curve` is assumed to be nonzero for arcs.
        this.curve = 0;
...
// The endpoint of a BÃ©zier curve. This is used when we want to draw a BÃ©zier curve in
// its entirety.
...
export class Arrow {
    ...
    /// Returns the points along the BÃ©zier curve associated to the arrow that intersect with the
    /// source and target.
    /// Returns either an array `[start, end]` or throws an error if the curve is invalid and has no
    /// nontrivial endpoints.
    find_endpoints() {
        const origin = this.origin();
        /// Finds the intersection of the (BÃ©zier or arc) curve with either the source or target.
        ...
        const find_endpoint = (endpoint_shape, endpoint_origin, prefer_min) => {
            ...
            if (intersections.length === 0) {
                // We should always have at least one intersection, as the BÃ©zier curve spans
                // the endpoints, so this is an error.
                console.error(
                    "No intersection found for BÃ©zier curve with endpoint.",
                    endpoint_shape,
                    curve,
                );
                // Bail out.
            ...
            // Check for BÃ©zier curves re-entering a rectangle. We don't check this case for arcs,
            ...
            if (this.style.shape === CONSTANTS.ARROW_SHAPE.BEZIER) {
                if (intersections.length > 1 && Curve.point_inside_polygon(
                    ...
                    throw new Error("The BÃ©zier re-enters an endpoint rectangle.");
                }
            }
    ...
    redraw() {
        ...
        // and simpler. However, an offset BÃ©zier curve is no longer a BÃ©zier curve, so we would
        // have to draw all these parallel paths manually, using line segments. This recursive
        // cutting approach allows us to make use of SVG's BÃ©zier abilities, which is nicer.
    ...
    edge_path(constants) {
        // Various arc lengths, which are used for drawing various parts of the BÃ©zier curve
        ...
        switch (this.style.body_style) {
            ...
            case CONSTANTS.ARROW_BODY_STYLE.SQUIGGLY:
                ...
                // This means we need to draw a BÃ©zier curve, but add little triangles along its
                ...
                // We now draw the BÃ©zier curve, augmented by triangular squiggles.
                for (
                    // The current arc length along the BÃ©zier curve we are tracing.
        ...
        // to avoid drawing the entire BÃ©zier curve. That is, the path of the BÃ©zier curve is
        // between the source and target for simplicity in calculation (namely, the BÃ©zier curves
    ...
    determine_label_position(constants) {
        ...
        // We calculate the label position for arcs differently from BÃ©zier curves: in this case, we
        ...
        // For BÃ©zier curves, we simply adjust up or down.
        ...
        // location. We first normalise the BÃ©zier curve or arc (flat BÃ©zier curves must be
