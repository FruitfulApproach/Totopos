...
QList<NodeHandles::Button> NodeHandles::buttons() const
{
	...
	if (arrow == nullptr)
	{
		shown << DeleteNode;   // an object: dragging draws, holding carries
		return shown;
	}
	shown << DrawArrow << DeleteNode;
	// the runner only when there is something to run with
	if (arrow->domain() != nullptr && arrow->domain()->holdsAnything())
		shown << Chase;
	return shown;
}
...
void NodeHandles::place()
{
	...
	m_out = QPointF(0, 1);
	const QRectF r = m_node->boundingRect();
...
void NodeHandles::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
	...
	for (int i = 0; i < shown.size(); ++i)
	{
		...
		case DrawArrow:
		{
			...
			painter->setBrush(hot ? QColor(79, 70, 229) : QColor(99, 102, 241, 235));
			painter->drawEllipse(c, kRadius, kRadius);
			QPolygonF head;
			head << c + m_out * 5.5 << c - m_out * 3.5 + along * 5 << c - m_out * 3.5 - along * 5;
		...
		{
			painter->setPen(QPen(QColor(255, 255, 255, 220), 1.2));
			painter->setBrush(hot ? QColor(185, 28, 28) : QColor(220, 38, 38, 235));
			painter->drawEllipse(c, kRadius, kRadius);
...
void NodeHandles::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
	...
	{
	case DrawArrow:  emit arrowRequested(node);  break;
	case DeleteNode: emit deleteRequested(node); break;
...
void NodeHandles::hoverMoveEvent(QGraphicsSceneHoverEvent* event)
{
	...
	if (m_hover >= 0)
	{
		...
		{
		case DrawArrow:  tip = "Draw an arrow from here: click, then click the object it goes to"; break;
		case DeleteNode: tip = "Delete this"; break;
