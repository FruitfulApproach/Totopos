...
LabelMoved::LabelMoved(const QString& description, Arrow* arrow, const QPointF& before, const QPointF& after)
...
{
	if (!m_arrow.isNull())
		m_arrow->setLabelOffset(m_before);
}

void LabelMoved::redo()
{
	if (!m_arrow.isNull())
		m_arrow->setLabelOffset(m_after);
}
