...
QList<Node*> RuleMatcher::apply(const Rule& rule, const RuleMatch& match, DiagramScene* diagram)
{
	...
	// what the rule said there is, there now is: drawn solid, and one step
	for (Node* node : made)
	...
	if (!made.isEmpty())
	{
		...
		diagram->recordCreation(
			QString("Applied %1%2").arg(rule.name(), where.isEmpty() ? QString() : QString(" with %1").arg(where.join(", "))),
			made);
	}
	return made;
}
