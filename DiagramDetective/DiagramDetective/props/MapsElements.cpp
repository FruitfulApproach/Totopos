...
void MapsElements::onFunctorRenamed()
{
	...
	for (QGraphicsItem* child : cod->childItems())
	{
		...
		if (node == nullptr || !isOurImage(node))
			continue;
		const QString source = node->data(ImageSourceKey).toString();
		node->setData(ImageFunctorKey, now);
		node->setId(applied(now, source));
	}
	m_syncing = false;
...
void MapsElements::stamp(Node* image, const QString& functor, const QString& sourceId) const
{
	image->setData(ImageFunctorKey, functor);
	image->setData(ImageSourceKey, sourceId);
...
Node* MapsElements::imageOf(Category* cod, const QString& functor, const QString& sourceId) const
{
	if (cod == nullptr)
		return nullptr;
	Node* byName = nullptr;
	const QString name = applied(functor, sourceId);
	for (QGraphicsItem* child : cod->childItems())
	{
		auto* node = dynamic_cast<Node*>(child);
		if (node == nullptr)
			continue;
		// ours, and of that source: found however it happens to be called
		if (isOurImage(node) && node->data(ImageSourceKey).toString() == sourceId)
			return node;
		if (byName == nullptr && node->id() == name)
			byName = node;   // drawn by the one-shot, or read from a file: adopt it
	}
	if (byName != nullptr)
		stamp(byName, functor, sourceId);
	return byName;
}
...
void MapsElements::onSourceBends(Arrow* source)
{
	...
	if (F == nullptr || cod == nullptr)
		return;
	auto* image = dynamic_cast<Arrow*>(imageOf(cod, F->id(), source->id()));
	if (image == nullptr)
		return;
	m_syncing = true;
...
void MapsElements::onImageBends(Arrow* image)
{
	...
	if (dom == nullptr)
		return;
	const QString sourceId = image->data(ImageSourceKey).toString();
	for (QGraphicsItem* child : dom->childItems())
	{
		auto* source = dynamic_cast<Arrow*>(child);
		if (source == nullptr || source->id() != sourceId)
			continue;
		m_syncing = true;
...
void MapsElements::onSourceMoved(Node* source, const QPointF& delta)
{
	...
	if (F == nullptr || cod == nullptr)
		return;
	Node* image = imageOf(cod, F->id(), source->id());
	if (image == nullptr)
		return;
...
void MapsElements::onImageMoved(Node* image, const QPointF& delta)
{
	...
	{
		auto* node = dynamic_cast<Node*>(child);
		if (node == nullptr || node->id() != sourceId)
			continue;
		m_syncing = true;
...
void MapsElements::sync()
{
	...
	QStringList live;   // the source labels that still exist
	...
	for (QGraphicsItem* child : dom->childItems())
	{
		...
		if (source == nullptr || dynamic_cast<Arrow*>(source) != nullptr || source->id().isEmpty())
			continue;
		live << source->id();
		Node* img = imageOf(cod, name, source->id());
		if (img == nullptr)
		{
